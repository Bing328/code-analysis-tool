...
class Config:
    ...
    FILE_TYPES = {
        # 源代码文件
        ".py": {"type": "source", "analyzer": "Python", "color": "#3572A5", "security_scan": True},
        ".java": {"type": "source", "analyzer": "Java", "color": "#B07219", "security_scan": True},
        ".c": {"type": "source", "analyzer": "C/C++", "color": "#555555", "security_scan": True},
        ".cpp": {"type": "source", "analyzer": "C/C++", "color": "#F34B7D", "security_scan": True},
        ".cs": {"type": "source", "analyzer": "C#", "color": "#178600", "security_scan": True},
        ".go": {"type": "source", "analyzer": "Go", "color": "#00ADD8", "security_scan": True},
        ".rs": {"type": "source", "analyzer": "Rust", "color": "#DEA584", "security_scan": True},
        ".js": {"type": "source", "analyzer": "JavaScript", "color": "#F1E05A", "security_scan": True},
        ".ts": {"type": "source", "analyzer": "TypeScript", "color": "#2B7489", "security_scan": True},
        ".php": {"type": "source", "analyzer": "PHP", "color": "#4F5D95", "security_scan": True},
        ".rb": {"type": "source", "analyzer": "Ruby", "color": "#701516", "security_scan": True},
        ".swift": {"type": "source", "analyzer": "Swift", "color": "#FFAC45", "security_scan": True},
        ".kt": {"type": "source", "analyzer": "Kotlin", "color": "#A97BFF", "security_scan": True},
        ".scala": {"type": "source", "analyzer": "Scala", "color": "#DC322F", "security_scan": True},
        ".sql": {"type": "source", "analyzer": "SQL", "color": "#E38C00", "security_scan": True},
        
        # 配置文件
        ".yaml": {"type": "config", "analyzer": "YAML", "color": "#CB171E", "security_scan": True},
        ".yml": {"type": "config", "analyzer": "YAML", "color": "#CB171E", "security_scan": True},
        ".json": {"type": "config", "analyzer": "JSON", "color": "#292929", "security_scan": True},
        ".toml": {"type": "config", "analyzer": "TOML", "color": "#9C4221", "security_scan": True},
        ".ini": {"type": "config", "analyzer": "INI", "color": "#7F7F7F", "security_scan": True},
        ".env": {"type": "config", "analyzer": "Env", "color": "#ECD53F", "security_scan": True},
        ".properties": {"type": "config", "analyzer": "Properties", "color": "#7F7F7F", "security_scan": True},
        
        # Docker文件
        "Dockerfile": {"type": "docker", "analyzer": "Docker", "color": "#2496ED", "security_scan": True},
        ".dockerfile": {"type": "docker", "analyzer": "Docker", "color": "#2496ED", "security_scan": True},
        "docker-compose.yml": {"type": "docker", "analyzer": "Docker Compose", "color": "#2496ED", "security_scan": True},
        "docker-compose.yaml": {"type": "docker", "analyzer": "Docker Compose", "color": "#2496ED", "security_scan": True},
        
        # 脚本文件
        ".sh": {"type": "script", "analyzer": "Shell", "color": "#89E051", "security_scan": True},
        ".bash": {"type": "script", "analyzer": "Bash", "color": "#89E051", "security_scan": True},
        ".zsh": {"type": "script", "analyzer": "Zsh", "color": "#89E051", "security_scan": True},
        ".ps1": {"type": "script", "analyzer": "PowerShell", "color": "#012456", "security_scan": True},
        ".bat": {"type": "script", "analyzer": "Batch", "color": "#C1F12E", "security_scan": True},
        ".cmd": {"type": "script", "analyzer": "Batch", "color": "#C1F12E", "security_scan": True},
        
        # 文档文件
        ".md": {"type": "document", "analyzer": "Markdown", "color": "#083FA1", "security_scan": False},
        ".rst": {"type": "document", "analyzer": "reStructuredText", "color": "#14B8A6", "security_scan": False},
        ".txt": {"type": "document", "analyzer": "Text", "color": "#6B7280", "security_scan": False},
        ".html": {"type": "document", "analyzer": "HTML", "color": "#E34C26", "security_scan": False},
        ".css": {"type": "document", "analyzer": "CSS", "color": "#563D7C", "security_scan": False},
        ".xml": {"type": "document", "analyzer": "XML", "color": "#0060AC", "security_scan": False},
    }
    
    # 二进制文件扩展名
    BINARY_EXTENSIONS = {
        '.exe', '.dll', '.so', '.dylib', '.bin', '.elf', '.msi', '.pdb',
        '.obj', '.class', '.jar', '.war', '.ear', '.pyc', '.pyo', '.pyd',
        '.whl', '.egg', '.o', '.a', '.lib', '.dll', '.sys', '.drv',
        '.ko', '.rpm', '.deb', '.apk', '.ipa', '.app', '.dmg', '.iso'
    }
    
    # 需要跳过的目录
    SKIP_DIRECTORIES = {
        # 虚拟环境
        'venv', 'env', 'virtualenv', 'myenv', '.env',
        # 包管理器目录
        'node_modules', 'bower_components', '.npm', '.yarn',
        # 版本控制
        '.git', '.svn', '.hg',
        # 构建目录
        'dist', 'build', 'bin', 'obj', 'out', 'output',
        # 缓存目录
        ...
        '.DS_Store', 'Thumbs.db',
        # 其他
        'logs', 'tmp', 'temp', 'cache', 'coverage', '.coverage'
    }
    ...
    SECURITY_PATTERNS = [
        ...
        (r'aws[_-]?(?:access[_-]?key|secret[_-]?key)\s*=\s*[\'"][^\'"]+[\'"]', "AWS凭证"),
...
{Colors.CYAN}{'='*60}{Colors.ENDC}
    ...
    def _get_ai_api_key(self):
        ...
        print(f"  1. 🔒 API密钥只在本地加密存储")
        print(f"  2. 🛡️  发送到服务器的内容经过隐私清理")
        print(f"  3. 📊 仅用于改进分析质量")
    ...
    def calculate_file_score(self, file_type: str, issues_count: int, 
                           warnings_count: int, is_binary: bool = False) -> int:
        """计算文件评分"""
        if is_binary:
            return 60
            
        base_scores = {
            "source": 95, "config": 85, "docker": 90,
            "script": 90, "document": 95, "binary": 60
        }
    ...
    def run_analysis(self):
        ...
        for idx, file_info in enumerate(files):
            ...
            print(f"\r[{idx+1}/{len(files)}] {progress:.1f}% - 分析中: {rel_path[:50]}...", end="")
    ...
    def generate_html_report(self):
        ...
        for result in self.results:
            ...
            row_class = "binary-row" if result.get('binary_warning') else ""
            type_color = result.get('color', '#6B7280')
        ...
        html_content = self._create_html_template(
            timestamp, avg_score, project_rank, rank_color, rank_description,
            security_count, quality_count, self.file_stats["ai_insights"],
            mode_descriptions.get(self.scan_mode, self.scan_mode), rows
        )
    ...
    def _create_html_template(self, timestamp, avg_score, project_rank, rank_color, 
                            rank_description, security_count, quality_count, 
                            ai_insights, mode_description, rows):
        """创建HTML模板"""
...
def main():
    ...
    try:
        ...
        if auditor.results:
            ...
            print(f"  2. 修复发现的安全问题")
            print(f"  3. 根据建议优化代码质量")
            if auditor.scan_mode == "online_ai":
                print(f"  4. AI分析报告已生成，查看智能建议")
        else:
