...
if [[ -z "${GRAFANA_ADMIN_PASSWORD:-}" ]]; then
    echo "❌ 错误：必须设置 GRAFANA_ADMIN_PASSWORD 环境变量"
    echo ""
    echo "请使用以下方式之一运行："
    echo ""
    echo "方法1：设置环境变量"
...
# ===== 3) 生成配置文件 =====
# Prometheus 配置
cat > prometheus/prometheus.yml << 'PROMETHEUS_EOF'
global:
  scrape_interval: 15s
...
# 告警规则
cat > prometheus/rules/node_rules.yml << 'RULES_EOF'
groups:
- name: node.rules
  rules:
  - alert: HostHighCPU
...
# Alertmanager 配置
cat > alertmanager/alertmanager.yml << 'ALERTMANAGER_EOF'
global:
  resolve_timeout: 5m
route:
  receiver: 'default'
...
cat > grafana/provisioning/datasources/datasource.yml << 'DATASOURCE_EOF'
apiVersion: 1
datasources:
- name: Prometheus
  type: prometheus
...
generate_remote_installer() {
    cat > /tmp/install_ne.sh << 'INSTALLER_EOF'
#!/bin/bash
set -e
...
echo ">>> 配置 Systemd 服务..."
cat << SERVICE_EOF | \$SUDO tee /etc/systemd/system/node_exporter.service > /dev/null
[Unit]
...
📊 访问面板:
   Grafana:      http://${HOST_IP}:3000  (账号: admin / 密码: 已通过环境变量设置)
   Prometheus:   http://${HOST_IP}:9090
   Alertmanager: http://${HOST_IP}:9093

🔐 安全提示:
   您的 Grafana 密码已通过环境变量设置，不在脚本中硬编码
   请妥善保管 GRAFANA_ADMIN_PASSWORD 环境变量
